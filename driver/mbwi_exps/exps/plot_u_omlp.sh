                ...
                for l in $L; do
                    ...
                    for t in $T; do
                        ...
                        if (( "$g" <= "$nr" )) && (( "$n" >= "$mn" )) && (( "$n" <= "$mx" )); then
                            ...
                            echo "plot \""$MYDIR"/tres_sorted.txt\" using 1:5 w lp t \"Feasible\", \""$MYDIR"/tres_sorted.txt\" using 1:6 w lp t \"M-BWI\", \""$MYDIR"/tres_sorted.txt\" using 1:7 w lp t \"FMLP\", \""$MYDIR"/tres_sorted.txt\" using 1:8 w lp t \"OMLP\"" >> $GFILE
                            gnuplot $GFILE&
                        fi
                        let index=index+1
                    done
